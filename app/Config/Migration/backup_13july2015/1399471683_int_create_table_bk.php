...
class IntCreateTable extends CakeMigration {
    ...
    public function before($direction) {
        if ($direction === 'up') {
            ...
            $clinics = $clinic->find('all', array('conditions' => array('clinics.id' => 3))); //condition need to remove after test ,array('conditions'=>array('clinics.id'=>1))
            foreach ($clinics as $cln) {

                $getcardfordelete = ClassRegistry::init('card_numbers');
                $gcarddelete = $getcardfordelete->find('first', array(
                    'conditions' => array(
                ...
                foreach ($gcarddelete as $gcarddel) {
                    ...
                    if (!empty($getuserdel)) {
                        $deletetransaction = ClassRegistry::init('transaction');
