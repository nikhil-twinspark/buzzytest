...
<?php } ?>
               ...
               <label >Zip:</label><span style='color:red;font-style:italic;padding-left:50px;' id="error_div_msg_postal_code" ></span>
									...
									<input 	class="form-control" type="text" name="postal_code" id="postal_code" value="<?=$postal_code?>" maxlength="6">
...
<?php } ?>
	...
	<?php	}
			...
			if (file_exists($uploadPath.'/'.$rewards['Reward']['description'])) {
				...
				<img src="<?=CDN.'img/rewards/'.$rewards['Reward']['api_user'].'/'.$rewards['Reward']['description']?>" width="240px" height="160px">
				<?php
			}else{
				if (file_exists($uploadPath."/noimage.jpg")) {
				?>

				<img src="<?=CDN.'img/rewards/'.$rewards['Reward']['api_user'].'/noimage.jpg';?>" width="240px" height="160px">
				<?php
				}else{
					?>
...
$.validator.addMethod("zipRegex", function(value, element) {
        ...
        submitHandler: function(form) {
            ...
            var r = confirm("Are you sure you want to redeem "+<?=$rewards['Reward']['points']?>+" points for this item?");
        if (r == true)
        {
            form.submit();
$('input[type="submit"]').attr('disabled','disabled');
            $('#progress_red_prd_div_page').html("<img src='<?php echo CDN; ?>img/loading.gif' > wait...");
        }else{
        return false;
        }
