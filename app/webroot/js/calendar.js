...
function Datepicker() {
	this._defaults = {
		...
		yearRange: 'c+20:c-20', // Range of years to show in drop-down: 'any' for direct text entry
			// or 'start:end', where start/end are '+-nn' for relative to today
			// or 'c+-nn' for relative to the currently selected date
			// or 'nnnn' for an absolute year
		startYear: 2016,
		endYear: 1930,
		shortYearCutoff: '+20', // Cutoff for two-digit year in the current century
...
$.extend(Datepicker.prototype, {
	...
	selectDate: function(target, elem) {
		...
		//add by sahoo on 2/2/2012
		if((target.name == 'eventStartDate') && (target.form.name == 'event')){
			//recalcDateDiff(target.value);
		}else if((target.name == 'eventEndDate') && (target.form.name == 'event')){
			recalcDateDiff(target.value);
		}
		//end here 
	},
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
		...
		if (yearRange == 'any') {
			...
			var year1 = inst.get('startYear');
			var year2 = inst.get('endYear');
			var start_year = year1;
			var end_year = year2;
